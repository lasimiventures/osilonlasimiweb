...
export function FilterSidebar({
  ...
  const content = (
    <div className="space-y-6">
      ...
      {/* Categories */}
      <div>
        <button onClick={() => toggleSection('categories')} className="flex items-center justify-between w-full text-sm font-medium text-slate-900 mb-2">
          Categories
          ...
          <div className="space-y-1">
            {categories.map((cat) => (
              <label key={cat.id} className="flex items-center gap-2 cursor-pointer group">
                ...
                <span className="text-xs text-slate-400 ml-auto">{cat.productCount}</span>
              </label>
            ))}
          </div>
        )}
      ...
      {/* Brands */}
      <div>
        <button onClick={() => toggleSection('brands')} className="flex items-center justify-between w-full text-sm font-medium text-slate-900 mb-2">
          Brands
          ...
          <div className="space-y-1 max-h-60 overflow-y-auto">
            {filteredBrands.map((brand) => (
              <label key={brand.id} className="flex items-center gap-2 cursor-pointer group">
                ...
                />
                <span className="text-sm text-slate-600 group-hover:text-slate-900 transition-colors">{brand.name}</span>
                <span className="text-xs text-slate-400 ml-auto">{brand.productCount}</span>
              </label>
            ))}
          </div>
        )}
      ...
      {/* Availability */}
      <div>
        <button onClick={() => toggleSection('availability')} className="flex items-center justify-between w-full text-sm font-medium text-slate-900 mb-2">
          Availability
          {sections.availability ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
        </button>
  ...
  return (
    <>
      ...
      <div className="lg:hidden mb-4">
        ...
        {mobileOpen && (
          <div className="mt-3 p-4 bg-white border border-slate-200 rounded-xl shadow-lg">
            {content}
          </div>
        )}
      </div>
      ...
      {/* Desktop Sidebar */}
      <div className="hidden lg:block sticky top-24 p-5 bg-white border border-slate-100 rounded-xl shadow-sm">
        {content}
      </div>
    </>
  );
