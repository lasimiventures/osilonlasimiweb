...
import { getActiveBanners } from '../lib/database';
...
export function Home() {
  ...
  useEffect(() => {
    getActiveBanners('hero').then(setHeroBanners).catch(() => {});
    getActiveBanners('promo').then(setPromoBanners).catch(() => {});
  }, []);

  const prevHero = useCallback(() =>
    setActiveHero(i => (i - 1 + heroBanners.length) % heroBanners.length), [heroBanners.length]);
  const nextHero = useCallback(() =>
    setActiveHero(i => (i + 1) % heroBanners.length), [heroBanners.length]);

  useEffect(() => {
    if (heroBanners.length < 2) return;
    const t = setInterval(nextHero, 5000);
    return () => clearInterval(t);
  }, [heroBanners.length, nextHero]);
  ...
  return (
    <>
      ...
      {heroBanners.length > 0 ? (
        <section className="relative bg-slate-900 overflow-hidden" style={{ minHeight: '420px' }}>
          <div className="relative h-[420px] lg:h-[500px]">
            {heroBanners.map((banner, idx) => (
              <div
                key={banner.id}
                className={`absolute inset-0 transition-opacity duration-700 ${idx === activeHero ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
              >
                {banner.imageUrl && (
                ...
                <div className="absolute inset-0 flex items-center">
                  <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 w-full">
                    ...
                    >
                      {banner.badgeText && (
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-brand-blue/90 text-white text-xs font-semibold rounded-full mb-4">
                          <BadgeCheck className="w-3.5 h-3.5" /> {banner.badgeText}
                        </div>
                      )}
                      ...
                      {banner.subtitle && (
                        <p className="text-base text-white/80 mb-6 max-w-md">{banner.subtitle}</p>
                      )}
                      <div className="flex flex-wrap gap-3">
                        {banner.ctaPrimaryText && banner.ctaPrimaryLink && (
                          <Link to={banner.ctaPrimaryLink} className="inline-flex items-center gap-2 px-6 py-3 bg-brand-blue text-white text-sm font-semibold rounded-lg hover:bg-brand-blue-dark transition-colors shadow-lg">
                            {banner.ctaPrimaryText} <ArrowRight className="w-4 h-4" />
                          </Link>
                        )}
                        {banner.ctaSecondaryText && banner.ctaSecondaryLink && (
                          <Link to={banner.ctaSecondaryLink} className="inline-flex items-center gap-2 px-6 py-3 bg-white/10 backdrop-blur-sm text-white text-sm font-semibold rounded-lg border border-white/30 hover:bg-white/20 transition-colors">
                ...
                <div className="absolute bottom-5 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2">
                  {heroBanners.map((_, idx) => (
                    <button
                      ...
                      className={`rounded-full transition-all duration-300 ${idx === activeHero ? 'w-6 h-2 bg-white' : 'w-2 h-2 bg-white/40 hover:bg-white/70'}`}
        ...
        <section className="relative bg-gradient-to-br from-slate-900 to-slate-800 overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12 py-14 lg:py-20">
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="flex-1 text-center lg:text-left">
                <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-brand-blue/20 text-brand-blue-pale text-xs font-semibold rounded-full mb-4">
                  <BadgeCheck className="w-3.5 h-3.5" /> Authorized Dealer in Kenya
                </div>
                <h1 className="text-3xl lg:text-5xl font-bold text-white leading-tight mb-4">
                  Quality Tech Products, <span className="text-brand-blue-pale">Delivered to You</span>
                </h1>
                <p className="text-sm text-slate-400 mb-6 max-w-lg mx-auto lg:mx-0">
                ...
                <div className="flex flex-wrap items-center justify-center lg:justify-start gap-3">
                  <Link to="/products" className="inline-flex items-center gap-2 px-6 py-3 bg-brand-blue text-white text-sm font-semibold rounded-lg hover:bg-brand-blue-dark transition-colors">
                    Shop Products <ArrowRight className="w-4 h-4" />
                  </Link>
                  <Link to="/request-quote" className="inline-flex items-center gap-2 px-6 py-3 text-sm font-semibold text-white border border-white/20 rounded-lg hover:bg-white/10 transition-colors">
                    Request a Quote
                  </Link>
                </div>
                <div className="flex items-center justify-center lg:justify-start gap-5 mt-6 text-xs text-slate-400">
      ...
      <section className="py-10 bg-slate-50 border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {(promoBanners.length > 0 ? promoBanners : promotions.map(p => ({
              id: p.id,
              title: p.title,
              subtitle: p.subtitle,
              badgeText: p.badge || null,
              imageUrl: p.image,
              ctaPrimaryText: p.ctaLabel,
              ctaPrimaryLink: p.ctaPath,
              ctaSecondaryText: null,
              ctaSecondaryLink: null,
              highlight: (p as { highlight?: string }).highlight,
            }))).map((item) => (
              <Link key={item.id} to={(item as { ctaPrimaryLink?: string }).ctaPrimaryLink ?? '/products'} className="group relative overflow-hidden rounded-xl border border-slate-200 bg-white hover:shadow-lg hover:shadow-slate-200/60 transition-all duration-300">
                <div className="relative h-44 overflow-hidden">
                  ...
                  {item.badgeText && (
                    <span className="absolute top-3 left-3 px-2.5 py-1 bg-blue-600 text-white text-xs font-semibold rounded-md">{item.badgeText}</span>
                  )}
                  {(item as { highlight?: string }).highlight && (
                    <span className="absolute top-3 right-3 px-2.5 py-1 bg-amber-500 text-white text-xs font-semibold rounded-md">{(item as { highlight?: string }).highlight}</span>
                  )}
                </div>
                <div className="p-4">
                  <h3 className="text-sm font-bold text-slate-900 mb-1 group-hover:text-blue-600 transition-colors">{item.title}</h3>
                  <p className="text-xs text-slate-500 leading-relaxed mb-3">{item.subtitle}</p>
                  <span className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600">
                    {(item as { ctaPrimaryText?: string | null }).ctaPrimaryText ?? 'Learn more'} <ArrowRight className="w-3 h-3" />
                  </span>
                </div>
              </Link>
            ))}
